...
これはグローバルなシングルトンです。ドッキングされたペインは一度に1つだけで、既に1つ
ドッキングされている状態で `sidebar-right` を指定した2回目の `plugin.pane.open` は
`ui_busy` を返します。`width`、`height`、`workspace_id`、`target_pane_id`、
...
アドレス指定可能なプラグインペインです。`plugin.pane.focus` と `plugin.pane.close` は
そのペイン ID に対して機能します。ドッキングされたプロセスは Herdr の再起動時に
