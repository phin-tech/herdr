...
`overlay`、`popup`、`split`、`tab`、`zoomed`、`sidebar-right` で上書きできます。
...
`placement = "sidebar-right"` は、プラグインペインをウィンドウの右端にドッキングし、
ポップアップのようにワークスペースの上に浮かぶのではなく、タイル化されたワークスペース
ペインと共存する永続的でリサイズ・折りたたみ可能なクロム領域として表示します。
複数のプラグインを同時にドッキングできます。既にドッキング済みのエントリポイントに
対する `plugin.pane.open` は、2つ目のコピーを起動するのではなくそのペインを前面に
出してフォーカスするため、トグル用のキーバインドは冪等です。
複数がドッキングされている場合、ドックにヘッダー行が表示され、クリック可能な `‹` と
`›` の矢印の間にアクティブなペインのマニフェスト `title` が表示されます。
`keys.next_dock_pane` と `keys.previous_dock_pane` はキーボードから同じ操作を行い、
既定では未割り当てです。
`width`、`height`、`workspace_id`、`target_pane_id`、
`direction` は受け付けません。ドックのサイズはプラグインではなくユーザー(ディバイダーの
ドラッグ、または `ui.dock_right_width`/`ui.dock_right_min_width`/`ui.dock_right_max_width`
設定キー)が制御します。ポップアップとは異なり、ドッキングされたペインは通常の
アドレス指定可能なプラグインペインです。`plugin.pane.focus`、`plugin.pane.close`、
`pane.read` はそのペイン ID に対して機能します。各ドッキングペインは
`dock_right:<plugin-id>.<entrypoint-id>` という安定した ID を持ち、隣のドックが
閉じても同じペインを指し続けます。`dock_right` 単体は現在表示中のペインの別名です。ドッキングされたプロセスは Herdr の再起動時に
復元されません。左サイドバーと同様に、ドックの幅と折りたたみ状態のみが永続化されます。

プラグインペインの起動時には `HERDR_PLUGIN_PLACEMENT` もエクスポートされ、
`plugin.pane.open` が受け付けるものと同じトークン(`overlay`、`popup`、`split`、
`tab`、`zoomed`、`sidebar-right`)が入ります。プラグインはこれを見て配置に合った
レイアウトを選べます。狭いドックは全幅のポップアップとは異なる表示が適しています。
また、このフォークが起動するすべてのペインとプラグインプロセスには
`HOARDER_ENV=1` が設定されます。
