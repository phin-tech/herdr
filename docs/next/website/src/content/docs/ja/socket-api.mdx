...
`sidebar-right` はペインをウィンドウの右端にドッキングし、ポップアップのようにワークスペースの上に浮かぶのではなく、永続的でリサイズ・折りたたみ可能な領域として表示します。これはグローバルなシングルトンです——既に1つドッキングされている状態で `sidebar-right` を指定した2回目の `plugin.pane.open` は `ui_busy` を返します——また `width`、`height`、`workspace_id`、`target_pane_id`、`direction` は受け付けません。ドックのサイズはユーザーが制御します(ディバイダーのドラッグ、または `ui.dock_right_width`/`ui.dock_right_min_width`/`ui.dock_right_max_width`)。ポップアップとは異なり、ドッキングされたペインは通常のアドレス指定可能なプラグインペインです。`plugin.pane.focus` と `plugin.pane.close` はそのペイン ID に対して操作できます。そのプロセスは Herdr の再起動後に復元されません。ドックの幅と折りたたみ状態のみが永続化されます。
