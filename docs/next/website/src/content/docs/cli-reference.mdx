...
```bash
herdr plugin pane open --plugin ID --entrypoint ID [--placement overlay|popup|split|tab|zoomed|sidebar-right] [--width SIZE] [--height SIZE] [--workspace ID] [--target-pane PANE] [--direction right|down] [--cwd PATH] [--env KEY=VALUE] [--focus|--no-focus]
...
in pane or agent APIs. `--placement sidebar-right` docks the pane to the right
edge of the window as a persistent, resizable, collapsible region instead of
floating over the workspace; it is a global singleton (a second `sidebar-right`
open while one is already docked returns `ui_busy`), does not accept `--width`,
`--height`, `--workspace`, `--target-pane`, or `--direction`, and — unlike a
popup — is a normal addressable plugin pane that `plugin pane focus` and
`plugin pane close` can target. Native non-terminal plugin panes are a later
surface.
