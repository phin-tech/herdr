...
`sidebar-right` 会把窗格停靠在窗口右侧边缘,成为一个持久的、可调整大小、可折叠的区域,而不是悬浮在工作区之上。它是一个全局单例——在已有窗格停靠时再次以 `sidebar-right` 调用 `plugin.pane.open` 会返回 `ui_busy`——并且不接受 `width`、`height`、`workspace_id`、`target_pane_id` 或 `direction`;停靠区域的大小由用户控制(拖动分隔线,或使用 `ui.dock_right_width`/`ui.dock_right_min_width`/`ui.dock_right_max_width`)。与弹窗不同,停靠窗格是一个普通的、可寻址的插件窗格:`plugin.pane.focus` 和 `plugin.pane.close` 可以操作它的窗格 ID。其进程不会在 Herdr 重启后恢复;只有停靠区域的宽度和折叠状态会被持久化。
