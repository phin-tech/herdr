...
它是一个全局单例:一次只能停靠一个窗格,在已有窗格停靠时再次以 `sidebar-right`
调用 `plugin.pane.open` 会返回 `ui_busy`。它不接受 `width`、`height`、
...
一个普通的、可寻址的插件窗格:`plugin.pane.focus` 和 `plugin.pane.close`
可以针对它的窗格 ID 操作。停靠的进程不会在 Herdr 重启后恢复;和左侧边栏一样,
只有停靠区域的宽度和折叠状态会被持久化。
