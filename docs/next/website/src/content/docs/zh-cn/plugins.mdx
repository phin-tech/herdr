...
可以用 `overlay`、`popup`、`split`、`tab`、`zoomed` 或 `sidebar-right` 覆盖清单的 placement。
...
`placement = "sidebar-right"` 会将插件窗格停靠在窗口右侧边缘,成为一个持久的、
可调整大小、可折叠的区域,与平铺的工作区窗格共存,而不是像弹窗那样悬浮其上。
多个插件可以同时停靠。对已停靠的入口点再次调用 `plugin.pane.open` 会聚焦并将其
置于前台,而不是再启动一个副本,因此切换类快捷键是幂等的。当停靠了多个窗格时,停靠区域会出现一行标题栏,在可点击的 `‹` 和 `›` 箭头之间显示
当前窗格清单中的 `title`;`keys.next_dock_pane` 和 `keys.previous_dock_pane`
可以用键盘完成同样的切换,默认未绑定。它不接受 `width`、`height`、
`workspace_id`、`target_pane_id` 或 `direction`——停靠区域的大小由用户控制
(拖动分隔线,或使用 `ui.dock_right_width`/`ui.dock_right_min_width`/
`ui.dock_right_max_width` 配置项),而不是由插件控制。与弹窗不同,停靠窗格是
一个普通的、可寻址的插件窗格:`plugin.pane.focus`、`plugin.pane.close` 和
`pane.read` 都可以针对它的窗格 ID 操作。每个停靠窗格都有形如
`dock_right:<plugin-id>.<entrypoint-id>` 的稳定 ID,相邻停靠窗格关闭时它仍指向
同一个窗格;裸写的 `dock_right` 是当前显示窗格的别名。停靠的进程不会在 Herdr 重启后恢复;和左侧边栏一样,
只有停靠区域的宽度和折叠状态会被持久化。

每次插件窗格启动还会导出 `HERDR_PLUGIN_PLACEMENT`,其值与 `plugin.pane.open` 接受的
令牌相同(`overlay`、`popup`、`split`、`tab`、`zoomed`、`sidebar-right`)。插件可以
据此选择适合其位置的布局——狭窄的停靠区域需要与全宽弹窗不同的界面。此外,本分支
启动的每个窗格和插件进程都会看到 `HOARDER_ENV=1`。
