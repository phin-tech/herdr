...
It is a global singleton: only one pane may be docked at a time, and a second
`plugin.pane.open` with `sidebar-right` returns `ui_busy` while one is already
open. It does not accept `width`, `height`, `workspace_id`, `target_pane_id`,
...
normal addressable plugin pane: `plugin.pane.focus` and `plugin.pane.close`
work against its pane ID. The docked process is not restored across a Herdr
