...
`overlay`, `popup`, `split`, `tab`, `zoomed`, or `sidebar-right`.
...
`placement = "sidebar-right"` docks a plugin pane to the right edge of the
window as a persistent, resizable, collapsible chrome region that coexists
with the tiled workspace panes, instead of floating over them like a popup.
Several plugins may hold docks at once. Opening an entrypoint that is already
docked focuses it and brings it to the front rather than spawning a second
copy, so a toggle keybinding is idempotent. When more than one is docked the
dock grows a header row showing the active pane's manifest `title` between
clickable `‹` and `›` arrows; the `keys.next_dock_pane` and
`keys.previous_dock_pane` bindings do the same from the keyboard and are unset
by default. It does not accept `width`,
`height`, `workspace_id`, `target_pane_id`,
or `direction` — the dock's size is controlled by the user (drag the divider,
or the `ui.dock_right_width`/`ui.dock_right_min_width`/`ui.dock_right_max_width`
config keys) rather than by the plugin. Unlike a popup, the docked pane is a
normal addressable plugin pane: `plugin.pane.focus`, `plugin.pane.close` and
`pane.read` work against its pane ID. Each docked pane has a stable ID of the
form `dock_right:<plugin-id>.<entrypoint-id>`, which keeps naming the same pane
when a neighbouring dock closes; the bare `dock_right` is an alias for whichever
pane is currently showing. The docked process is not restored across a Herdr
restart — only the dock's width and collapsed state persist, the same as the
left sidebar.

Every plugin pane launch also exports `HERDR_PLUGIN_PLACEMENT`, holding the
same token `plugin.pane.open` accepts (`overlay`, `popup`, `split`, `tab`,
`zoomed`, `sidebar-right`). A plugin can branch on it to render a layout that
suits where it was put — a narrow dock wants different chrome than a
full-width popup. Panes docked to the right also see `HOARDER_ENV=1`, as does
every pane and plugin process this fork spawns.
